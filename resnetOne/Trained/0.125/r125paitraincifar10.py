...
def main():
    ...
    dataset1 = datasets.CIFAR10(root='./data', train=True, download=True, transform=transform_train)
    dataset2 = datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_test)
    ...
    # excel_path = f"train_results_r125_pai_train.xlsx"
    ...
    while(continue_pruning==True):
        if(continue_pruning==True):
            ...
            for c_epochs in range(1, args.epochs + 1):
                ...
                val_loss_val, val_acc_val, model, optimizer, scheduler, trainingComplete = evaluate(model, device, val_loader, optimizer, scheduler, args)
                if trainingComplete:
                    break
                scheduler.step()
            ...
            # torch.save(model, f"models125PAITrain/{pruningTimes}_pruned_model.pth")
            # pruningTimes += 1
            exit(1)
