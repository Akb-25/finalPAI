...
def main(model):    
  ...
  while(continue_pruning==True):
    ...
    if(continue_pruning==True):
      ...
      torch.save(model, f"modelsInit2_1/{pruningTimes}_pruned_model.pth")
...
if __name__ == "__main__":
  ...
  model = PN.loadPAIModel(model, '25ModelInit.pt')
#   model = torch.load(f"modelsTrained1_{iteration}/{iteration}_pruned_model.pth", weights_only=False)
  ...
  excel_path = f"training_results2_1Init.xlsx"
