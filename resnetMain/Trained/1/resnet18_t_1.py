...
def main(model):    
  ...
  #______________________Custom_Regularize the model___________________________
      ...
      # model = best_model
      torch.save(model, f"modelsTrained1_1/{pruningTimes}_pruned_model.pth")
      best_test_acc = 0
      best_model = None
...
if __name__ == "__main__":
  ...
  prune_limits = [1] * 2 * 5 * 1
  prune_value = [1] * 2 * 1 + [2] * 2 * 1 + [4] * 2 * 1 + [8] * 2 * 5  
  ...
  model = PN.loadPAIModel(model, 'PB/best_model_pai.pt')
